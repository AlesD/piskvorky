...
ARG_INTEGER: INT -> type(INTEGER);
ARG_IGNORED: (WS | COMMENT) -> skip;
ARG_EOL: NEW_LINE -> mode(DEFAULT_MODE), type(EOL);
...
KVP_EOL: NEW_LINE -> mode(DEFAULT_MODE), type(EOL);

mode TEXT_MODE;

TEXT_IGNORED: (WS | COMMENT) -> skip; // Goes first to remove leading WS
WORD: ~[ \t\n#]+;
TEXT_EOL: NEW_LINE -> mode(DEFAULT_MODE), type(EOL);